	...
	}
	{
		...
		if (TIFFGetField(in, TIFFTAG_NUMBEROFINKS, &ninks)) {
			...
			if (TIFFGetField(in, TIFFTAG_INKNAMES, &inknames)) {
				...
				TIFFSetField(out, TIFFTAG_INKNAMES, (uint32)inknameslen, inknames);
			}
		}
	}
	{
...
cpStripToTile(uint8* out, uint8* in,
    tsize_t rows, tsize_t cols, tsize_t outskew, tsize_t inskew)
{
    while (rows-- > 0) {
        tsize_t j = cols;
        while (j-- > 0) *out++ = *in++;
        out += outskew; in += inskew;
    }
}

static void
cpContigBufToSeparateBuf(uint8* out, uint8* in,
    tsize_t rows, tsize_t cols, tsize_t outskew, tsize_t inskew, tsample_t spp,
...
DECLAREreadFunc(readSeparateStripsIntoBuffer)
{
	...
	_TIFFfree(scanline); return status;
}
...
DECLAREreadFunc(readContigTilesIntoBuffer)
{
    ...
    tsize_t tilew  = TIFFTileRowSize(in);
    tsize_t imagew = TIFFScanlineSize(in);
    tsize_t iskew = imagew - tilew;
    uint32 tw, tl; uint32 row;
    uint8* bufp = (uint8*) buf;

    (void) spp;
    tilebuf = _TIFFmalloc(tilesize);
    if (tilebuf == 0) return 0;
    _TIFFmemset(tilebuf, 0, tilesize);
    (void) TIFFGetField(in, TIFFTAG_TILEWIDTH, &tw);
    (void) TIFFGetField(in, TIFFTAG_TILELENGTH, &tl);

    for (row = 0; row < imagelength; row += tl) {
        tsize_t nrow = (row+tl > imagelength) ? imagelength-row : tl;
        tsize_t colb = 0; uint32 col;
        for (col = 0; col < imagewidth; col += tw) {
            ...
            if (colb + tilew > imagew) {
                tsize_t width = imagew - colb;
                tsize_t oskew = tilew - width;
...
DECLAREreadFunc(readSeparateTilesIntoBuffer)
{
    int status = 1;
    tsize_t tilew  = TIFFTileRowSize(in);
    tsize_t imagew = TIFFRasterScanlineSize(in);
    tsize_t iskew  = imagew - tilew*spp;
    tsize_t tilesize = TIFFTileSize(in);
    tdata_t tilebuf;
    uint32 tw, tl; uint32 row;
    uint8* bufp = (uint8*) buf;
...
{
    tsize_t rowsize = imagewidth * spp;
    uint32 rowsperstrip;
...
DECLAREwriteFunc(writeBufferToContigTiles)
{
    ...
    tsize_t imagew = TIFFScanlineSize(out);
    tsize_t iskew = imagew - tilew;
    tsize_t tilesize = TIFFTileSize(out);
    tdata_t obuf;
    uint8* bufp = (uint8*) buf;
    uint32 tl, tw; uint32 row;
...
DECLAREwriteFunc(writeBufferToSeparateTiles)
{
    tsize_t tilew  = TIFFTileRowSize(out);
    tsize_t imagew = TIFFScanlineSize(out);
    tsize_t iimagew = TIFFRasterScanlineSize(out);
    tsize_t iskew = iimagew - tilew*spp;
    tsize_t tilesize = TIFFTileSize(out);
    tdata_t obuf;
    uint32 tl, tw; uint32 row;
    uint8* bufp = (uint8*) buf;
