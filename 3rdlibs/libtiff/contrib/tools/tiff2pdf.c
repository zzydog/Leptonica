...
typedef struct {
    ...
    /* Here, using tsize_t instead (void warning C4244) */
    tsize_t pdf_startxref;
...
#define TIFF2PDF_KEYWORDS_SIZE 512
    ...
    /* Here, should we use tsize_t instead */
    tsize_t* pdf_xrefoffsets;
    ...
    /* 0 (default) : do not interpolate, 1 : interpolate */
...
tsize_t t2p_readwrite_pdf_image(T2P* t2p, TIFF* input, TIFF* output){
    ...
    tstrip_t j=0;
    tstrip_t stripcount=0;
    /* Should be defined as tstrip_t */
    tstrip_t sepstripcount=0;
    tsize_t stripsize=0;
...
tsize_t t2p_write_pdf(T2P* t2p, TIFF* input, TIFF* output){
    ...
    t2p->pdf_xrefoffsets = _TIFFmalloc(TIFFSafeMultiply(tmsize_t,t2p->pdf_xrefcount,sizeof(tsize_t)) );
    if(t2p->pdf_xrefoffsets==NULL){
        TIFFError(
            TIFF2PDF_MODULE, 
            "Can't allocate %u bytes of memory for t2p_write_pdf", 
            (unsigned int) (t2p->pdf_xrefcount * sizeof(tsize_t)) );
        t2p->t2p_error = T2P_ERR_ERROR;
        return(written);
    }
    t2p->pdf_xrefcount=0;
