...
static int  dump_data  (FILE *, int, char *, unsigned char *, tsize_t);
...
static int  dump_buffer(FILE *, int, tsize_t, tsize_t, tsize_t, unsigned char *);
...
static int readContigTilesIntoBuffer (TIFF* in, uint8* buf, 
  ...
  tsize_t tile_rowsize  = TIFFTileRowSize(in);
  tsize_t src_offset, dst_offset;
                                    ...
                                    struct dump_opts* dump)
{
  ...
  tsize_t tile_rowsize  = TIFFTileRowSize(out);
  uint8* bufp = (uint8*) buf;
                            ...
                            unsigned int *imagelist, unsigned int *image_count )
{
    ...
    uint32 start; tsize_t end; unsigned int i, j;
#if !HAVE_DECL_OPTARG
    extern int   optind;
    extern char* optarg;
#endif
    ...
    *mp++ = 'w'; *mp = '\0';
        ...
        "ac:d:e:f:hil:m:p:r:stvw:z:BCD:E:F:H:I:J:K:LMN:O:P:R:S:U:V:X:Y:Z:")) != -1)
    {
        ...
        case 'c':
            ...
            break;
        case 'd':
            start = (uint32)strtoul(optarg, NULL, 0); /* initial IFD offset */
            if (start == 0)
            {
        ...
        case 'h':	
            usage(); break;
        case 'i':
            ignore = TRUE; break;                                   /* ignore errors */
        case 'l':
            outtiled = TRUE; *deftilelength = atoi(optarg); break;  /* tile length */
        ...
        case 's': /* generate stripped output */
            outtiled = FALSE; break;
        case 't': /* generate tiled output */
            outtiled = TRUE; break;
        case 'v': 
            TIFFError("Library Release", "%s", TIFFGetVersion());
            TIFFError("Tiffcrop version", "%s, last updated: %s", tiffcrop_version_id, tiffcrop_rev_date);
            ...
            exit (0); break;
        case 'w': /* tile width */
            outtiled = TRUE; *deftilewidth = atoi(optarg); break;
        ...
        case 'E': /* edge reference */
            ...
            {
                case 't': crop_data->edge_ref = EDGE_TOP;    break;
                case 'l': crop_data->edge_ref = EDGE_LEFT;   break;
                case 'r': crop_data->edge_ref = EDGE_RIGHT;  break;
                case 'b': crop_data->edge_ref = EDGE_BOTTOM; break;
                default : 
        ...
        case 'F': /* flip eg mirror image or cropped segment, M was already used */
            ...
            switch (tolower((int) optarg[0]))
            {
                case 'h': crop_data->mirror = MIRROR_HORIZ; break;
                case 'v': crop_data->mirror = MIRROR_VERT; break;
                case 'b': crop_data->mirror = MIRROR_BOTH; break;
        ...
        case 'O': /* page orientation */ 
            switch (tolower((int) optarg[0])) {
                case 'a': page->orient = ORIENTATION_AUTO; break;
                case 'p': page->orient = ORIENTATION_PORTRAIT; break;
                case 'l': page->orient = ORIENTATION_LANDSCAPE; break;
        ...
        case 'P': /* page size selection */ 
            ...
            if (get_page_geometry (optarg, page)) {
                ...
                exit (-1);
            } else { page->mode |= PAGE_MODE_PAPERSIZE; }
            break;
        case 'R': /* rotate image or cropped segment */
            crop_data->crop_mode |= CROP_ROTATE;
            switch (strtoul(optarg, NULL, 0))
            {
            case 90 : crop_data->rotation = (uint16)90 ; break;
            case 180: crop_data->rotation = (uint16)180; break;
            case 270: crop_data->rotation = (uint16)270; break;
...
main(int argc, char* argv[])
{
    ...
    while (optind < argc - 1)
    {
        ...
        if (dirnum > (total_images))
        {
            ...
            if (out != NULL) (void) TIFFClose(out);
            return (1);
        }
        ...
        while (end_of_input == FALSE)
        {
            ...
            if (dump.format != DUMP_NONE)
            {
                ...
                if (length > 0)
                {
                    if (dump.infile != NULL) fclose (dump.infile);
                ...
                if (length > 0)
                {
                    if (dump.outfile != NULL) fclose (dump.outfile);
            ...
            */
            ...
            {  /* Whole image or sections not based on output page size */
                 ...
                 * in the read_buffer
                 */
                if (crop_buff != NULL) sect_src = crop_buff;
                else sect_src = read_buff;
            ...
            if (image_count == 0) dirnum++;
            else { dirnum = (tdir_t)(imagelist[next_image] - 1); next_image++; }

            if (dirnum == MAX_IMAGES - 1) dirnum = TIFFNumberOfDirectories(in) - 1;

            if (!TIFFSetDirectory(in, (tdir_t)dirnum)) end_of_input = TRUE;
        }
        TIFFClose(in); optind++;
    }

    /* If we did not use the read buffer as the crop buffer */
    if (read_buff) _TIFFfree(read_buff);
    if (crop_buff) _TIFFfree(crop_buff);
    if (sect_buff) _TIFFfree(sect_buff);
    ...
    TIFFClose(out); return (0);
} /* end main */


/* Debugging functions */
static int dump_data (FILE *dumpfile, int format, char *dump_tag, unsigned char *data, tsize_t count)
  {
  int j, k;
  uint32 i;
  char  dump_array[10];
...
static int dump_buffer (FILE* dumpfile, int format, tsize_t rows, tsize_t width, 
                 tsize_t row, unsigned char *buff)
{
    tsize_t j, k; tsize_t i; unsigned char * dump_ptr;
    ...
    for (i = 0; i < rows; i++)
    {
        ...
        if (k > 0) dump_data (dumpfile, format, "", dump_ptr, k);
    }
    return (0);
}
                            ...
                            tsample_t count, uint32 start, uint32 end)
{
    ...
    for (col = start; col < end; col++)
    {
        ...
        for (sindex = sample; (sindex < spp) && (sindex < (sample + count)); sindex++)
        {
            ...
            if (ready_bits >= 32)
            {
                bytebuff1 = (uint8)(buff2 >> 56); *dst++ = bytebuff1;
                bytebuff2 = (uint8)(buff2 >> 48); *dst++ = bytebuff2;
                bytebuff3 = (uint8)(buff2 >> 40); *dst++ = bytebuff3;
                bytebuff4 = (uint8)(buff2 >> 32); *dst++ = bytebuff4;
                ready_bits -= 32;
    ...
    while (ready_bits > 0)
    {
        bytebuff1 = (buff2 >> 56); *dst++ = bytebuff1;
        buff2 = (buff2 << 8); ready_bits -= 8;
    }
                            ...
                            FILE *dumpfile, int format, int level)
{
    ...
    for (row = 0; row < rows; row++)
    {
        ...
        if ((dumpfile != NULL) && (level == 3))
        {
            ...
            dump_data (dumpfile, format, "Src   bits ", src, (tsize_t)4);
                                         ...
                                         struct dump_opts *dump)
{
    ...
    if (rps > length) rps = length;

    bytes_per_sample = (bps + 7) / 8; 
    bytes_per_pixel  = ((bps * spp) + 7) / 8;
    if (bytes_per_pixel < (bytes_per_sample + 1)) shift_width = bytes_per_pixel;
    else shift_width = bytes_per_sample + 1;
     ...
     */
    nstrips = TIFFNumberOfStrips(in); strips_per_sample = nstrips /spp;
    ...
    for (s = 0; (s < spp) && (s < MAX_SAMPLES); s++)
    {
        ...
        if (!buff)
        {
            ...
            for (i = 0; i < s; i++) _TIFFfree (srcbuffs[i]);
            return 0;
        }
        srcbuffs[s] = buff;
    }
    ...
    for (j = 0; (j < strips_per_sample) && (result == 1); j++)
    {
        ...
        if (rps > rows_this_strip) rps = rows_this_strip;
        ...
        if ((rows_processed + rps) > length) {
            rows_processed = length; rps = length - rows_processed;
        } else rows_processed += rps;
    }
...
getCropOffsets(struct image_data *image, struct crop_mask *crop, struct dump_opts *dump)
{
       ...
       (crop->crop_mode & CROP_WIDTH))
    {
        ...
        need_buff = TRUE; crop->selections = crop->regions;
        /* Regions are only calculated from top and left edges with no margins */
        if (crop->crop_mode & CROP_REGIONS) return (0);
    ...
    } else crop->selections = crop->zones;
    ...
    for (i = 0; i < crop->zones; i++)
    {
        ...
        case EDGE_BOTTOM: /* width from left, zones from bottom to top */
            ...
            // warning C4244: '=' : conversion from 'double' to 'int32'
            //test = offsets.endy - (uint32)(offsets.crop_length * 1.0 * seg / total);
            test = offsets.endy - (uint32)(offsets.crop_length*1.0*seg / total);
            if (test < 1 ) crop->regionlist[i].y1 = 0;
            else crop->regionlist[i].y1 = test + 1;
            // warning C4244: '=' : conversion from 'double' to 'int32'
        ...
        case EDGE_RIGHT: /* zones from right to left, length from top */
            ...
            // warning C4244: '=' : conversion from 'double' to 'int32'
            /*crop->regionlist[i].x1 = offsets.startx +
                (offsets.crop_width * (total-seg) * 1.0 / total);
            test = offsets.startx + 
        ...
        buffsize = (uint32)((((zwidth*image->bps*image->spp)+7) / 8) * (zlength + 1));
...
loadImage(TIFF* in, struct image_data *image, struct dump_opts *dump, unsigned char **read_ptr)
{
    ...
    case COMPRESSION_NONE:	/* 1  dump mode */
        strcpy (compressionid, "None/dump"); break;         
    case COMPRESSION_CCITTRLE:	  /* 2 CCITT modified Huffman RLE */
        strcpy (compressionid, "Huffman RLE"); break;         
    case COMPRESSION_CCITTFAX3:	  /* 3 CCITT Group 3 fax encoding */
        strcpy (compressionid, "Group3 Fax"); break;         
    case COMPRESSION_CCITTFAX4:	  /* 4 CCITT Group 4 fax encoding */
        strcpy (compressionid, "Group4 Fax"); break;         
    case COMPRESSION_LZW:	  /* 5 Lempel-Ziv  & Welch */
        strcpy (compressionid, "LZW"); break;         
    case COMPRESSION_OJPEG:	  /* 6 !6.0 JPEG */
        strcpy (compressionid, "Old Jpeg"); break;         
    case COMPRESSION_JPEG:	  /* 7 %JPEG DCT compression */
        strcpy (compressionid, "New Jpeg"); break;         
    case COMPRESSION_NEXT:	  /* 32766 NeXT 2-bit RLE */
        strcpy (compressionid, "Next RLE"); break;         
    case COMPRESSION_CCITTRLEW:   /* 32771 #1 w/ word alignment */
        strcpy (compressionid, "CITTRLEW"); break;         
    case COMPRESSION_PACKBITS:	  /* 32773 Macintosh RLE */
        strcpy (compressionid, "Mac Packbits"); break;         
    case COMPRESSION_THUNDERSCAN: /* 32809 ThunderScan RLE */
        strcpy (compressionid, "Thunderscan"); break;         
    case COMPRESSION_IT8CTPAD:	  /* 32895 IT8 CT w/padding */
        strcpy (compressionid, "IT8 padded"); break;         
    case COMPRESSION_IT8LW:	  /* 32896 IT8 Linework RLE */
        strcpy (compressionid, "IT8 RLE"); break;         
    case COMPRESSION_IT8MP:	  /* 32897 IT8 Monochrome picture */
        strcpy (compressionid, "IT8 mono"); break;         
    case COMPRESSION_IT8BL:	  /* 32898 IT8 Binary line art */
        strcpy (compressionid, "IT8 lineart"); break;         
    case COMPRESSION_PIXARFILM:	  /* 32908 Pixar companded 10bit LZW */
        strcpy (compressionid, "Pixar 10 bit"); break;         
    case COMPRESSION_PIXARLOG:	  /* 32909 Pixar companded 11bit ZIP */
        strcpy (compressionid, "Pixar 11bit"); break;         
    case COMPRESSION_DEFLATE:	  /* 32946 Deflate compression */
        strcpy (compressionid, "Deflate"); break;         
    case COMPRESSION_ADOBE_DEFLATE: /* 8 Deflate compression */
        strcpy (compressionid, "Adobe deflate"); break;         
    default:
        strcpy (compressionid, "None/unknown"); break;         
    }
    ...
    case ORIENTATION_TOPLEFT:
         image->adjustments = 0; break;
    case ORIENTATION_TOPRIGHT:
         image->adjustments = MIRROR_HORIZ; break;
    case ORIENTATION_BOTRIGHT:
         image->adjustments = ROTATECW_180; break;
    case ORIENTATION_BOTLEFT:
         image->adjustments = MIRROR_VERT;  break;
    case ORIENTATION_LEFTTOP:
         image->adjustments = MIRROR_VERT | ROTATECW_90; break;
    case ORIENTATION_RIGHTTOP:
         image->adjustments = ROTATECW_90; break;
    case ORIENTATION_RIGHTBOT:
         image->adjustments = MIRROR_VERT | ROTATECW_270; break; 
    case ORIENTATION_LEFTBOT:
         image->adjustments = ROTATECW_270; break;
    default:
         image->adjustments = 0; image->orientation = ORIENTATION_TOPLEFT;
    }
    ...
    if (TIFFIsTiled(in))
    {
        ...
        tile_rowsize  = TIFFTileRowSize(in); buffsize = tlsize * ntiles;
    ...
    prev_readsize = buffsize; *read_ptr = read_buff;
    ...
    case TILE:
        if (planar == PLANARCONFIG_CONTIG) {
            if (!(readContigTilesIntoBuffer(
                in, read_buff, length, width, tw, tl, spp, bps))) {
    ...
    */
    ...
    if (compression != (uint16)-1) {
        ...
        if (input_compression == COMPRESSION_OJPEG) {
            ...
            /* Use the compression from the input file */
    ...
    if(fillorder != 0) TIFFSetField(out, TIFFTAG_FILLORDER, fillorder);
    else CopyTag(TIFFTAG_FILLORDER, 1, TIFF_SHORT);
    ...
    if (config != (uint16) -1) TIFFSetField(out, TIFFTAG_PLANARCONFIG, config);
    else CopyField(TIFFTAG_PLANARCONFIG, config);
    if (spp <= 4) CopyTag(TIFFTAG_TRANSFERFUNCTION, 4, TIFF_SHORT);
    ...
    { 
        uint32 len32; void** data;
        if (TIFFGetField(in, TIFFTAG_ICCPROFILE, &len32, &data))
            TIFFSetField(out, TIFFTAG_ICCPROFILE, len32, data);
    }
    { 
        uint16 ninks; const char* inknames;
        if (TIFFGetField(in, TIFFTAG_NUMBEROFINKS, &ninks)) {
            TIFFSetField(out, TIFFTAG_NUMBEROFINKS, ninks);
            if (TIFFGetField(in, TIFFTAG_INKNAMES, &inknames)) {
                tsize_t inknameslen = strlen(inknames) + 1;
                const char* cp = inknames;
                while (ninks > 1) {
                   cp = strchr(cp, '\0');
                   if (cp) { cp++; inknameslen += (strlen(cp) + 1); }
                   ninks--;
                }
                TIFFSetField(
                    out, TIFFTAG_INKNAMES, (uint32)inknameslen, inknames);
            }
        }
    }
    {
                      ...
                      unsigned char **read_buff_ptr, struct buffinfo seg_buffs[])
{
    ...
    if (crop->img_mode == COMPOSITE_IMAGES)
    {
        ...
        if (crop->crop_mode & CROP_INVERT) {
            ...
            case INVERT_DATA_AND_TAG:
                if (invertImage(
                    image->photometric, image->spp, image->bps, crop->combined_width, 
    ...
    else  /* Separated Images */
    {
        ...
        for (i = 0; i < crop->selections; i++)
        {
            cropsize = crop->bufftotal; crop_buff = seg_buffs[i].buffer; 
            if (!crop_buff) {
                crop_buff = (unsigned char *)_TIFFmalloc(cropsize);
            } else {
                prev_cropsize = seg_buffs[0].size;
                  ...
                  unsigned char *crop_buff, int pagenum, int total_pages)
{
    ...
    if (outtiled) {
        ...
        if (rowsperstrip == (uint32) 0) {
            ...
            if (compression != COMPRESSION_JPEG)
                if (rowsperstrip > length) rowsperstrip = length;
        } else if (rowsperstrip == (uint32) -1) rowsperstrip = length;
        TIFFSetField(out, TIFFTAG_ROWSPERSTRIP, rowsperstrip);
    }
                         ...
                         uint32 length,   uint32 col, uint8 *src, uint8 *dst)
{
    ...
    for (row = 0; row < length ; row++)
    {
        ...
        for (sample = 0; sample < spp; sample++)
        {
            ...
            /* If we have a full buffer's worth, write it out */
            if (ready_bits >= 8) {
                *dst++ = buff2; buff2 = buff1; ready_bits -= 8;
            } else {
                buff2 = (buff2 | (buff1 >> ready_bits));
            }
            ready_bits += bps;
                      ...
                      uint8 *ibuff, uint8 *obuff)
{
    ...
    ready_bits = 0; mask_bits =  (uint64)-1 >> (64 - bps); dst = obuff;
