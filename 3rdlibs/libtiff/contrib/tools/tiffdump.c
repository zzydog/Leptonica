	...
	for (dp = (uint8*)dirmem, n = dircount; n > 0; n--) {
...
#endif
		if (type >= NWIDTHS) typewidth = 0;
		else typewidth = datawidth[type];
		...
		if (!bigtiff)
		{
			if (datasizeoverflow || datasize>4)
			{
				...
				if (swabflag) TIFFSwabLong(&dataoffset32);
		...
		else
		{
			if (datasizeoverflow || datasize>8)
			{
				...
				if (swabflag) TIFFSwabLong8(&dataoffset);
			}
			dp += sizeof(uint64);
		}
		...
		if (!datafits)
		{
			...
			if (datamem) {
                ...
                /* You should know that the way we read file here is not safe */
				else if (read(fd, datamem, (size_t)datasize) != datasize)
				{
					Error("Read error accessing tag %u value", tag);
					_TIFFfree(datamem);
					datamem = NULL;
				}
			} else Error("No space for data for tag %u",tag);
		}
		if (datamem)
		{
			if (swabflag)
