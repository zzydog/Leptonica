...
int  psMaskImage(FILE *, TIFF *, int, int, uint32 *, uint32, uint32,
            double, double, double, double, double, double, double);
...
main(int argc, char* argv[])
{
    ...
    case 'o':
        switch (optarg[0])
        {
            ...
            case '7':
            case '8':
            case '9': diroff = (uint32) strtoul(optarg, NULL, 0); break;
            default : 
                TIFFError ("-o", "Offset must be a numeric value."); exit (1);
        }
        break;
    ...
    case 'r':
        if (strcmp (optarg, "auto") == 0) {
            rotation = 0; auto_rotate = TRUE;
        } else {
            rotation = atoi(optarg); auto_rotate = FALSE;
        }
        switch (rotation)
        {
            case   0:
 ...
 */
static void
PhotoshopBanner(FILE* fd, unsigned long w, unsigned long h, 
                unsigned int bs, unsigned int nc, char* startline)
{
    fprintf(fd, "%%ImageData: %lu %lu %u %u 0 %u 2 \"",
        (unsigned long) w, (unsigned long) h, bitspersample, nc, bs);
...
setupPageState(TIFF* tif, uint32* pw, uint32* ph, double* pprw, double* pprh)
{
    ...
    case RESUNIT_NONE:	/* Subsequent code assumes we have converted to inches! */
        ...
        break;
    default:
        /* Last ditch guess for unspecified RESUNIT case
     ...
     */
    if ((xres > 1.0) && (xres != PS_UNIT_SIZE)) *pprw = PSUNITS(*pw, xres);
    else *pprw = PSUNITS(*pw, PS_UNIT_SIZE);
    if ((yres > 1.0) && (yres != PS_UNIT_SIZE)) *pprh = PSUNITS(*ph, yres);
    else *pprh = PSUNITS(*ph, PS_UNIT_SIZE);
}
...
/* Use uint32 instead (Fixed warning C4244) */
static uint32 ps_bytesperrow;
static uint32 tf_bytesperrow;
...
int get_subimage_count(double pagewidth, double pageheight,
                    ...
                    int *ximages, int *yimages, int rotation, double scale)
{
    int pages = 1;
    ...
    pages = (*ximages) * (*yimages); return (pages);
}
    ...
    case 180: 
        ...
        if (imagewidth <= pagewidth) {
            /* Intra2net: Crop page at the bottom instead of the top 
                (-> output starts at the top). Only do this in non-page-split mode */
            if (imageheight <= splitheight) {
                /* Note: Will be negative for images longer than page size */
                ytran = pageheight - imageheight; 
            }
        }
...
int psMaskImage(FILE *fd, TIFF *tif, int rotation, int center,
                uint32 *npages, uint32 pixwidth, uint32 pixheight,
   ...
   * maxPageHeight since this makes life way too complicated. */
    if (auto_rotate)
    {
        ...
        *scale = (xscale < yscale) ? xscale : yscale;
         ...
         */
        if (!npages) PSHead(fd, reqwidth, reqheight, ox, oy);
   ...
   */
    ...
    case 180: 
        ...
        {  /* Viewport clipped to maxPageHeight or maxPageWidth */
            ...
            else *scale = 1.0;      /* Image clipped but not scaled */
        ...
        else   /* Viewport not clipped to maxPageHeight or maxPageWidth */
        {
            ...
            {   /* Image scaled  */
                xscale = (reqwidth - left_offset) / pswidth;
                yscale = (reqheight - bottom_offset) / psheight;
                view_width = reqwidth; view_height = reqheight;
            }
            else
            {  /* Image not scaled  */
                xscale = (pswidth - left_offset)/pswidth;
                yscale = (psheight - bottom_offset)/psheight;
                view_width = pswidth; view_height = psheight;
            }
        }
        break;
    case  90:
    case 270: 
        ...
        else /* Viewport not clipped to maxPageHeight or maxPageWidth */
        {
            ...
            else
            {
                ...
                view_width = psheight; view_height = pswidth;
            }
        }
        break;
    default:  
        ...
        if (pgheight != 0) {/* User has set PageHeight with -h flag */
            ...
            */
            ...
            switch (rotation)
            {
                default:
                case   0:
                case 180: *view_height = psheight; break;
                case  90:
                case 270: *view_height = pswidth;  break;
            }
        }
    }
        ...
        if (pgwidth != 0)  /* User has set PageWidth with -w flag */
        {
            ...
            *view_width = pgwidth * PS_UNIT_SIZE; 
            ...
            switch (rotation)
            {
                default:
                case   0:
                case 180: *view_width = pswidth;  break;
                case  90:
                /* (*view_height / psheight) * psheight; */
                case 270: *view_width = psheight; break;
            }
        }
    }
    ...
    do {
        ...
        if (psStart(fd, npages, auto_rotate, &rotation, &scale, ox, oy,
                pgwidth, pgheight, view_width, view_height, pswidth, psheight,
                 left_offset, bottom_offset)) return (-1);
        ...
        if (checkImage(tif))  /* Aborts if unsupported image parameters */
        {
            ...
            if ((maxPageHeight) || (maxPageWidth)  || (pgwidth != 0) || (pgheight != 0))
            {
                ...
                else  /* N.B. Setting maxPageHeight no longer sets pgheight */
                {
                    if (pgwidth != 0 || pgheight != 0)
                    {
                        /* User did not specify a maxium page height or width 
        ...
        if (generateEPSF) break;
        if (auto_rotate) rotation = 0;
...
PSHead(FILE *fd, double pagewidth, double pageheight, double xoff, double yoff)
{
    ...
    if (generateEPSF == TRUE) fprintf(fd, "%%%%Pages: 1 1\n");
    else fprintf(fd, "%%%%Pages: (atend)\n");
	fprintf(fd, "%%%%EndComments\n");
    if (generateEPSF == FALSE)
    {
        fprintf(fd, "%%%%BeginSetup\n");
        if (PSduplex) fprintf(fd, "%s", DuplexPreamble);
        if (PStumble) fprintf(fd, "%s", TumblePreamble);
...
PS_Lvl2colorspace(FILE* fd, TIFF* tif)
{
    ...
    {
    case PHOTOMETRIC_SEPARATED:
        colorspace_p = "CMYK"; break;
    case PHOTOMETRIC_RGB:
        colorspace_p = "RGB";  break;
    default:
        colorspace_p = "Gray"; break;
    }
...
#undef CVT
    ...
    for (i = 0; i < num_colors; i++) {
        if (ascii85) {
            ...
            fputs((i % 8) ? " " : "\n  ", fd);
            fprintf(fd, "%02x%02x%02x", rmap[i], gmap[i], bmap[i]);
        }
    }
    if (ascii85) Ascii85Flush(fd);
    else fputs(">\n", fd);
    fputs("] setcolorspace\n", fd);
}
...
PS_Lvl2ImageDict(FILE* fd, TIFF* tif, uint32 w, uint32 h)
{
    ...
    if ( useImagemask && (bitspersample == 1) ) imageOp = "imagemask";
    ...
    if (repeat_count > 1) fprintf(fd, "%d { %% repeat\n", repeat_count);
...
PS_FlipBytes(unsigned char* buf, tsize_t count)
{
    ...
    if (count <= 0 || bitspersample <= 8) return;
...
PSDataBW(FILE* fd, TIFF* tif, uint32 w, uint32 h)
{
    ...
    for (s = 0; s < TIFFNumberOfStrips(tif); s++) {
        ...
        if (ascii85) {
            ...
            if ( ascii85_l > 0 ) fwrite( ascii85_p, ascii85_l, 1, fd );
#else
            while (cc-- > 0) Ascii85Put(*cp++, fd);
...
PSRawDataBW(FILE* fd, TIFF* tif, uint32 w, uint32 h)
{
    ...
    for ( s = 0; ++s < (tstrip_t)tf_numberstrips; ) {
        if ( bc[s] > bufsize ) bufsize = (uint32) bc[s];
    }
...
Ascii85Init(void)
{
    ascii85breaklen = 2*MAXLINE; ascii85count = 0;
}
